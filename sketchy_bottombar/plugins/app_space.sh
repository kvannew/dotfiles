...
create_icons() {
  sketchy_bottombar --set space.$1 label="$(create_label "$1")"
}
...
update_icons() {
  ...
  sketchy_bottombar --animate tanh 10                                   \
...
create_label() {
  ...
  for APP in "${APPS[@]}"; do
    # Add icon
    LABEL+=$("$HOME/.config/sketchy_bottombar/plugins/app_icon.sh" "$APP")
    # Set up badge
    BADGE="$(set_badge $APP)"
...
set_space_label() {
  sketchy_bottombar --set $NAME icon="$@"
}
