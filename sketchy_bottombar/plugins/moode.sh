...
render_bar_item() {
  sketchy_bottombar --set $NAME label="$CURRENT_ARTIST: $CURRENT_SONG"
}

# render_popup() {
#   sketchy_bottombar --set music.cover "${music_cover[@]}"   \
...
update() {
  ...
  if [ "$PLAYER_STATE" = "play" ]; then
    sketchy_bottombar --set $NAME drawing=on icon=􀊆

    render_bar_item
    # render_popup

  else
    sketchy_bottombar --set $NAME icon=􀊄
    # popup off
