...
update() {
  ...
  *)
  ...
  sketchy_bottombar --set $NAME icon=$ICON icon.color=$COLOR
  sketchy_bottombar --set $NAME.value label="$PERCENTAGE%"
}

label_toggle() {
  update

  DRAWING_STATE=$(sketchy_bottombar --query $NAME.value | jq -r '.label.drawing')
  ...
  sketchy_bottombar --set $NAME.value label.drawing=$DRAWING \
