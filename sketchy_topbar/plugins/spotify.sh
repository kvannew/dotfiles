...
repeat () 
{
  ...
  if [ "$REPEAT" = "false" ]; then
    sketchy_topbar -m --set spotify.repeat icon.highlight=on
    osascript -e 'tell application "Spotify" to set repeating to true'
  else 
    sketchy_topbar -m --set spotify.repeat icon.highlight=off
...
shuffle () 
{
  ...
  if [ "$SHUFFLE" = "false" ]; then
    sketchy_topbar -m --set spotify.shuffle icon.highlight=on
    osascript -e 'tell application "Spotify" to set shuffling to true'
  else 
    sketchy_topbar -m --set spotify.shuffle icon.highlight=off
...
popup () {
  sketchy_topbar --set spotify.anchor popup.drawing=$1
}
